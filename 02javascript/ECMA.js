...
函数与方法的区别：与其他类绑定的函数就是方法；函数可以直接调用，方法需要通过对象来调用；this：谁调用了当前的函数或者方法，this就是谁，因此函数this输出的是window，方法this输出的就是当前调用的对象。
*/

//构造函数 构造函数和工厂函数一样，都是专门用于创建对象的，构造函数的本质时工厂函数的简写；构造函数的函数名称首字母必须大写；构造函数只能通过new来调用；
